...
class Evaluator:
    ...
    def evaluate_retrieval(
        ...
        for qa in qa_pairs:
            ...
            # 自顶向下：优先使用社区的 doc_ids（文档级），若无则从 text_unit_ids 提取 doc_id
            # 每个社区贡献最多 TOP_DOWN_PER_COMM 个 doc_id，总上限 TOP_DOWN_CAP 个
            # 两个版本均使用相同的截断规则，确保对等性
            top_down_ids: List[str] = []
            for comm_hit in result.top_down_hits:
                count = 0
                # 优先使用 doc_ids（v3 新增的文档级 ID，能直接匹配 ground-truth）
                candidate_ids = comm_hit.doc_ids if comm_hit.doc_ids else []
                if not candidate_ids:
                    # 降级：从 text_unit_ids（sent_id 格式）提取 doc_id
                    # sent_id 格式：{doc_title}-p{NNN}-s{NNN}
                    seen_doc: set = set()
                    for tid in comm_hit.text_unit_ids:
                        tid_str = str(tid)
                        # 去掉 -pXXX-sXXX 后缀提取 doc_id
                        doc_id = re.sub(r"-p\d+-s\d+$", "", tid_str)
                        if not doc_id:
                            doc_id = tid_str
