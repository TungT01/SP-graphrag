...
目标函数：
    J = Q_leiden - lambda * H_structure

其中：
    Q_leiden   : 标准 Leiden 模块度增益（衡量语义相似性）
    H_structure: 社区内节点物理来源的香农熵（衡量物理分散度）
    lambda     : 退火系数，随层级升高从极大值衰减至 0

v5 变更（渐进合并架构）：
    1. 接入 EdgeSchedule：每层开始前注入对应层次的边，
       使 Leiden 能够发现跨句子、跨段落、跨文档的社区结构。
       注意：聚合后节点 ID 变为 super_xxx_lN，需要维护
       original_to_current 映射表将原始 ID 转换为当前超节点 ID。
    2. 移除 lambda_val < 1e-6 终止条件：
       lambda 衰减仅影响 delta_H 的惩罚力度，不影响层数。
       层数由图的收敛性自然决定（num_communities <= 1 或不变）。
...
算法流程（每一层）：
    1. 分层加边（v5 新增）：根据 EdgeSchedule 注入当前层次的边
    2. 局部移动阶段（Local Moving）：
       对每个节点，遍历其邻居所在社区，
       计算移动后的 delta_J = delta_Q - lambda * delta_H，
       若 delta_J > 0 则执行移动。
    3. 细化阶段（Refinement）：
...
def hierarchical_leiden_constrained(
    ...
    v5 变更：
        - 新增 edge_schedule 参数：每层开始前注入对应层次的边
        - 移除 lambda_val < 1e-6 终止条件：层数由收敛性决定
        - 维护 original_to_current 映射：将原始节点 ID 映射到当前超节点 ID，
          用于 EdgeSchedule 在高层正确注入边
    ...
    edge_schedule : EdgeSchedule, optional
        ...
        EdgeSchedule 中的边使用原始节点 ID，注入时会自动映射到当前超节点 ID。
    ...
    -------
    HierarchicalCommunityResult
        层次化聚类结果，包含每层的社区分配、结构熵和 lambda 值。
    """
    ...
    # v5 新增：维护原始节点 ID -> 当前超节点 ID 的映射
    # 用于 EdgeSchedule 在高层正确注入边
    original_to_current: Dict[str, str] = {
        node: node for node in graph.nodes()
    }
    ...
    while True:
        ...
        # v5 新增：分层加边
        # EdgeSchedule 中的边使用原始节点 ID，需要映射到当前层的超节点 ID
        if edge_schedule is not None:
            new_edges = edge_schedule.get_edges_for_level(level)
            for orig_u, orig_v, w in new_edges:
                # 将原始节点 ID 映射到当前层的超节点 ID
                curr_u = original_to_current.get(orig_u)
                curr_v = original_to_current.get(orig_v)
                if curr_u is None or curr_v is None:
                    continue
                if not current_graph.has_node(curr_u) or not current_graph.has_node(curr_v):
                    continue
                if curr_u == curr_v:
                    continue
                if current_graph.has_edge(curr_u, curr_v):
                    current_graph[curr_u][curr_v]["weight"] += w
                else:
                    current_graph.add_edge(curr_u, curr_v, weight=w)
        ...
        # v5 新增：更新 original_to_current 映射
        # 每个原始节点现在对应其所在社区的超节点
        new_original_to_current: Dict[str, str] = {}
        for current_node, super_node in node_to_super.items():
            for original_node in super_to_originals.get(current_node, {current_node}):
                new_original_to_current[original_node] = super_node
