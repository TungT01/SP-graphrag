...
**MultiHop-RAG**：609 篇新闻文章，2556 个多跳 QA 对。

| 实验版本 | 采样数 | 有效QA | 实体数 | 关系数 |
|---------|-------|--------|--------|--------|
| v7 | 200 | 169 | 13,716 | 18,044 |
| v8（全量） | 500 | 429 | 26,289 | 34,532 |
...
### 5.3 实验结果（v8，n=500，429 有效 QA）

| 组号 | λ_0 | avg_H | MRR | P@5 | R@5 | NDCG@10 | Para-MRR | 层数 | MRR 95%CI |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| [0] Baseline | 0 | 0.0000 | 0.4478 | 0.1944 | 0.3990 | 0.3624 | 0.4275 | 3 | [0.409, 0.484] |
| [1] ES only | 0 | 0.1366 | 0.4469 | 0.1953 | 0.4009 | 0.3607 | 0.4275 | 8 | [0.407, 0.485] |
| [2] Weak | 0.001 | 0.1149 | 0.4478 | 0.1953 | 0.4015 | 0.3612 | 0.4275 | 8 | [0.408, 0.485] |
| [3] **Med** | **0.003** | **0.0891** | 0.4463 | 0.1939 | 0.3984 | 0.3605 | 0.4275 | 10 | [0.407, 0.485] |
| [4] +PathA | 0.001 | 0.1198 | 0.4464 | 0.1944 | 0.3994 | 0.3613 | 0.4275 | 9 | [0.407, 0.485] |
| [5] +CrossDoc | 0.001 | 0.1493 | 0.4428 | 0.1944 | 0.3974 | 0.3596 | 0.4275 | 12 | [0.403, 0.481] |

**新增指标说明**：
- **Para-MRR**：段落级 MRR，衡量 BottomUp 路径的物理文本块命中率
- **MRR 95%CI**：1000 次 Bootstrap 重采样的置信区间，各组 CI 高度重叠，确认差异不显著

### 5.4 核心结论

1. **λ 正确惩罚了熵**：avg_H 随 λ 增大单调递减（0.1366 → 0.0891），物理来源分布更纯净。
2. **检索质量无损（Bootstrap 证实）**：所有组 MRR 95%CI 高度重叠（[0.403, 0.485]），差异在统计上不显著。
3. **BottomUp 路径完全独立于社区结构**：Para-MRR 六组完全一致（0.4275），证实 BottomUp 是纯物理检索，不受 Leiden 影响。
4. **CrossDoc 边轻微伤害**：跨文档边引入跨主题噪声，MRR 最低（0.4428，-1.1%）。
5. **Weak（λ=0.001）表现最均衡**：R@5 微升 +0.6%，MRR 持平，avg_H 有效降低。
6. **推荐配置**：`λ_init=0.001–0.003, decay_rate=0.5, anchor_granularity="para", include_cross_doc=False`
...
| v7 | 完成全实验流水线 + 六组消融实验（n=200） | 验证方案有效性：熵可控 + 检索无损 |
| v8 | 修复 BottomUp anchor bug + Bootstrap CI + Para-Hit 指标 + 全量实验（n=500） | 统计显著性验证 + 段落级精确度量化 |

### 6.1 已知问题与限制

1. ~~**BottomUp 锚点加权 Bug**~~：**已在 v8 修复**。`entity_chunks` 现在将 sent_id 截断为 para_id 格式（`re.sub(r"-s\d+$", "", sent_id)`），与 text_units 的 chunk_id 格式匹配。修复后 Para-MRR=0.4275，确认锚点加权正常工作。

2. **spaCy 抽取质量**：依存句法三元组在新闻文本上尚可（平均每篇文章 ~43 个实体，n=500 时），但对非英文或专业领域文本质量下降。

3. ~~**实验评估采样**~~：**v8 已扩展至 n=500（429 有效 QA，16.8% 采样率）**，并引入 Bootstrap 1000 次重采样的 95% CI。统计功效已显著提升。
...
### 数据规模参考（v8 全量）
```
MultiHop-RAG: 609 articles, ~14479 paragraphs, ~32365 sentences
Graph (n=500): 26289 nodes, 34532 edges (after extraction)
EdgeSchedule: +4290 injected edges (intra-para + intra-doc)
Communities: Level 0 ~35000, Level 10 ~40
Runtime: ~35 minutes total pipeline (M1 Mac, n=500)
```

---

*文档版本：v8.0 | 更新时间：2026-05-08 | 适用代码快照：当前 main 分支（含 v8 BottomUp 修复 + Bootstrap CI + Para-Hit）*